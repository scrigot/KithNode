import Link from "next/link";
import { prisma, meUserEmail } from "@/lib/me/db";

// Picker: choose who you're meeting, then jump to their prep brief.
export default async function MePrepPicker() {
  const userId = meUserEmail();
  const contacts = await prisma.meContact.findMany({
    where: { userId },
    orderBy: { name: "asc" },
    select: { id: true, name: true, firmName: true, title: true },
    take: 1000,
  });

  return (
    <div className="max-w-3xl mx-auto px-8 py-10">
      <h1 className="text-2xl font-semibold tracking-tight">Coffee-chat prep</h1>
      <p className="mt-1 text-[14px] text-[#B7AFA7]">
        Pick who you&rsquo;re meeting — get a one-screen brief: who they are, your history,
        what they care about, 5 sharp questions, and the ask.
      </p>

      {contacts.length === 0 ? (
        <div className="mt-8 rounded-xl border border-dashed border-[#38332F] bg-[#201E1D] p-10 text-center">
          <p className="text-[15px] text-[#C9C2BB]">No contacts yet</p>
          <p className="mt-1 text-[13px] text-[#8A8077]">Import your network on the Contacts tab first.</p>
        </div>
      ) : (
        <div className="mt-6 rounded-xl border border-[#38332F] overflow-hidden">
          {contacts.map((c) => (
            <Link
              key={c.id}
              href={`/me/prep/${c.id}`}
              className="flex items-center justify-between gap-3 border-t border-[#2E2A27] first:border-t-0 px-4 py-3 hover:bg-[#232020] transition-colors"
            >
              <div className="min-w-0">
                <div className="text-[14px] font-medium text-white truncate">{c.name}</div>
                <div className="text-[12px] text-[#9C948C] truncate">
                  {[c.title, c.firmName].filter(Boolean).join(" · ") || "—"}
                </div>
              </div>
              <span className="text-[12px] text-[#E8643C] shrink-0">Prep →</span>
            </Link>
          ))}
        </div>
      )}
    </div>
  );
}
