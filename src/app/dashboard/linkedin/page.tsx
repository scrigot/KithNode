...
const inputClass = "w-full border border-white/[0.1] bg-background px-3 py-2 text-sm outline-none focus:border-primary/60";
...
export default function LinkedInWorkspacePage() {
  ...
  return (
    <div className="min-h-full bg-bg-primary">
      ...
      <aside className="hidden w-72 shrink-0 border-r border-white/[0.08] bg-card md:flex md:flex-col">
        <div className="border-b border-white/[0.08] p-4">
          ...
          <div className="mt-3 grid grid-cols-2 gap-2">
            ...
            <button onClick={() => setImportOpen(true)} disabled={Boolean(busy)} className="flex items-center justify-center gap-1 border border-white/10 px-2 py-2 text-xs disabled:opacity-50"><FileJson size={13} /> Import</button>
          </div>
        </div>
        <div className="flex-1 space-y-1 overflow-y-auto p-2">
          {profiles.map((profile) => (
            <button key={profile.id} onClick={() => openProfile(profile.id)} className={`w-full border p-3 text-left ${selected?.id === profile.id ? "border-primary/40 bg-primary/10" : "border-transparent hover:border-white/10 hover:bg-background"}`}>
        ...
        <div className="border-t border-white/[0.08] p-3 text-[10px] leading-relaxed text-muted-foreground"><ShieldCheck size={12} className="mb-1 text-accent-green" />KithNode does not log into LinkedIn or publish changes. You review and copy edits manually.</div>
      </aside>
      ...
      <main className="min-w-0 flex-1 p-4 lg:p-6">
        {importOpen && (
          <section className="mb-4 border border-primary/30 bg-card p-4">
            ...
            <div className="mt-2 flex gap-2"><button onClick={submitImport} disabled={!importJson.trim() || Boolean(busy)} className="bg-primary px-3 py-2 text-xs font-semibold text-white disabled:opacity-50">Import copy</button><button onClick={() => setImportOpen(false)} className="border border-white/10 px-3 py-2 text-xs">Cancel</button></div>
          </section>
        )}

        {!selected ? (
          <div className="mx-auto mt-24 max-w-xl border border-white/[0.08] bg-card p-8 text-center"><IdCard className="mx-auto text-primary" size={32} /><h2 className="mt-3 text-lg font-bold">Build your LinkedIn source of truth</h2><p className="mt-2 text-sm text-muted-foreground">Keep multiple versions, audit every section, and preserve the history of every revision.</p><button onClick={() => createProfile("manual")} className="mt-5 bg-primary px-4 py-2 text-sm font-semibold text-white">Create profile copy</button></div>
        ) : (
          <div className="mx-auto max-w-7xl">
            ...
            {tab === "editor" && (
              <div className="space-y-4">
                ...
                {SECTION_DEFINITIONS.map((definition) => (
                  <details key={definition.key} className="border border-white/[0.08] bg-card" open={definition.key === "experience" || definition.key === "education" || definition.key === "skills"}>
                    <summary className="cursor-pointer list-none p-4"><div className="flex items-center justify-between gap-3"><div><h2 className="text-sm font-bold">{definition.label} <span className="font-mono text-xs text-primary">{content.sections[definition.key].length}</span></h2><p className="mt-1 text-xs text-muted-foreground">{definition.hint}</p></div><button type="button" onClick={(event) => { event.preventDefault(); addItem(definition.key); }} className="flex shrink-0 items-center gap-1 border border-white/10 px-2 py-1 text-xs text-primary"><Plus size={12} /> Add</button></div></summary>
                    <div className="space-y-3 border-t border-white/[0.08] p-4">
                      ...
                      {content.sections[definition.key].map((item, index) => (
                        <article key={item.id} className="border border-white/[0.08] bg-background p-3">
                ...
                <section className="border border-white/[0.08] bg-card p-4"><label><span className={labelClass}>Private workspace notes</span><textarea value={content.notes} onChange={(event) => setContent((current) => ({ ...current, notes: event.target.value }))} rows={4} className={inputClass} /></label></section>
                <section className="sticky bottom-3 z-10 flex flex-wrap items-center gap-3 border border-primary/20 bg-card/95 p-3 shadow-xl backdrop-blur"><input value={changeSummary} onChange={(event) => setChangeSummary(event.target.value)} placeholder="What changed? (optional)" className={`min-w-60 flex-1 ${inputClass}`} /><button onClick={saveProfile} disabled={Boolean(busy)} className="flex items-center gap-1 bg-primary px-4 py-2 text-xs font-bold text-white"><Save size={13} /> Save revision</button></section>
              </div>
            )}
            ...
            {tab === "history" && (
              <section className="border border-white/[0.08] bg-card p-4"><div className="flex items-center gap-2"><History size={16} className="text-primary" /><h2 className="text-sm font-bold">Revision history</h2></div><p className="mt-1 text-xs text-muted-foreground">Every save is immutable. Restoring an old version creates a new revision; it never erases history.</p><div className="mt-4 space-y-2">{revisions.map((revision) => <div key={revision.id} className="flex flex-wrap items-center justify-between gap-3 border border-white/[0.08] bg-background p-3"><div><p className="text-sm font-semibold">Version {revision.version} · {revision.changeSummary || revision.source}</p><p className="mt-1 text-[11px] text-muted-foreground">{new Date(revision.createdAt).toLocaleString()} · score {revision.score}/100 · {revision.source}</p></div><button onClick={() => restoreRevision(revision)} disabled={Boolean(busy)} className="flex items-center gap-1 border border-white/10 px-3 py-2 text-xs disabled:opacity-50">{busy === `restore:${revision.id}` ? <Loader2 size={12} className="animate-spin" /> : <RotateCcw size={12} />} Restore</button></div>)}</div></section>
            )}
          </div>
        )}
      </main>
