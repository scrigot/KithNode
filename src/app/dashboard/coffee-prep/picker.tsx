    ...
    <div className="mx-auto max-w-5xl p-5">
      ...
      <section className="border border-border-soft bg-card p-4">
        <div className="grid gap-2 md:grid-cols-[minmax(0,1fr)_260px]">
          <label className="flex items-center gap-2 border border-border bg-background px-3"><Search size={14} className="text-muted-foreground" /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search name, company, or title…" className="h-10 min-w-0 flex-1 bg-transparent text-sm outline-none" /></label>
          <select value={tier} onChange={(event) => setTier(event.target.value)} className="h-10 border border-border bg-background px-3 text-sm outline-none"><option value="">All relationships</option><option value="kith">Kith</option><option value="hot">Hot</option><option value="warm">Warm</option><option value="monitor">Monitor</option><option value="cold">Cold</option></select>
        </div>
      ...
      {loading && <div className="mt-4 flex items-center gap-2 border border-border-soft bg-card p-6 text-sm text-muted-foreground"><Loader2 size={15} className="animate-spin" />Loading your network…</div>}
      {error && <div className="mt-4 border border-accent-red/30 bg-accent-red/10 p-4 text-sm text-accent-red">{error}</div>}
      {!loading && !error && filtered.length === 0 && <div className="mt-4 border border-dashed border-border bg-card p-10 text-center"><p className="text-sm">No matching contacts.</p><p className="mt-1 text-xs text-muted-foreground">Clear the filters or import contacts first.</p></div>}
      <div className="mt-4 divide-y divide-border-soft border border-border-soft bg-card">
        {filtered.map((contact) => (
          <Link key={contact.id} href={`/dashboard/coffee-prep/${contact.id}`} className="flex items-center justify-between gap-4 p-4 transition-colors hover:bg-surface-selected">
            <div className="min-w-0"><p className="truncate text-sm font-semibold">{contact.name}</p><p className="mt-0.5 truncate text-xs text-muted-foreground">{[contact.title, contact.company?.name].filter(Boolean).join(" · ") || "No role recorded"}</p><div className="mt-2 flex gap-2 text-[10px] uppercase tracking-wider text-muted-foreground">{contact.score?.tier && <span className="border border-border px-1.5 py-0.5">{contact.score.tier}</span>}{contact.pipeline_stage && <span className="border border-primary/20 bg-primary/5 px-1.5 py-0.5 text-primary">{contact.pipeline_stage}</span>}</div></div>
