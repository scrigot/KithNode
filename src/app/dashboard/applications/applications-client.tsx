...
const inputClass = "min-h-11 w-full rounded-lg border border-border bg-white px-3 text-sm text-text-primary outline-none placeholder:text-text-muted focus:border-primary";
...
export function ApplicationsClient() {
  ...
  async function updateOpportunity(id: string, patch: Record<string, unknown>, successMessage?: string) {
    setSaving(true);
    try {
      const response = await apiFetch(`/api/applications/${id}`, { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify(patch) });
      const body = await response.json().catch(() => ({}));
  ...
  async function addEvent(event: React.FormEvent<HTMLFormElement>) {
    ...
    if (!selected) return;
    const form = new FormData(event.currentTarget);
    const response = await apiFetch(`/api/applications/${selected.id}/events`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ type: "note", title: form.get("title"), detail: form.get("detail") }) });
    const body = await response.json().catch(() => ({}));
    if (response.ok) {
  ...
  async function generateResume() {
    if (!selected) return;
    setSaving(true);
    const response = await apiFetch(`/api/applications/${selected.id}/resume`, { method: "POST" });
    const body = await response.json().catch(() => ({}));
    setSaving(false);
  ...
  async function archiveSelected() {
    if (!selected) return;
    const response = await apiFetch(`/api/applications/${selected.id}`, { method: "DELETE" });
    if (response.ok) {
  ...
  return (
    <div className="min-h-full bg-canvas">
      <WorkspaceHeader eyebrow="Recruiting pipeline" title="Applications" description="Jobs, internships, clubs, schools, scholarships, and programs—with every deadline, relationship, document, and next action linked." actions={<><button type="button" onClick={() => setView("table")} aria-pressed={view === "table"} className={`min-h-10 rounded-lg border px-3 text-sm font-medium ${view === "table" ? "border-primary/30 bg-primary-soft text-primary" : "border-border bg-white text-text-secondary"}`}><TableProperties className="mr-2 inline h-4 w-4" />Table</button><button type="button" onClick={() => setView("board")} aria-pressed={view === "board"} className={`min-h-10 rounded-lg border px-3 text-sm font-medium ${view === "board" ? "border-primary/30 bg-primary-soft text-primary" : "border-border bg-white text-text-secondary"}`}><Columns3 className="mr-2 inline h-4 w-4" />Board</button><button type="button" onClick={() => setAddOpen(true)} className="min-h-10 rounded-lg bg-primary px-4 text-sm font-semibold text-white hover:bg-primary-hover"><Plus className="mr-2 inline h-4 w-4" />Add application</button></>} />
      <MetricStrip items={metrics} />

      {notice ? <div className="flex items-center justify-between border-b border-primary/20 bg-primary-soft px-4 py-3 text-sm text-primary" role="status"><span>{notice}</span><button type="button" onClick={() => setNotice("")} className="min-h-11 px-3 font-bold">Dismiss</button></div> : null}
      {undo ? <div className="fixed bottom-20 right-4 z-40 flex items-center gap-3 border border-border bg-white p-3 shadow-2xl lg:bottom-4"><span className="text-sm text-text-primary">Application moved</span><button type="button" onClick={() => { const item = items.find((row) => row.id === undo.id); if (item) void moveStatus(item, undo.status, false); setUndo(null); }} className="min-h-11 px-3 font-bold text-primary"><Undo2 className="mr-2 inline h-4 w-4" />Undo</button></div> : null}

      <section className="mx-auto mt-5 w-[calc(100%-2.5rem)] max-w-[1130px] rounded-xl border border-border bg-white p-3" aria-label="Application filters">
      ...
      {staleCount ? <div className="border-b border-warning/20 bg-warning-soft px-4 py-2 text-sm text-warning sm:px-6">{staleCount} saved listing{staleCount === 1 ? " is" : "s are"} more than seven days old. Open the official listing before applying.</div> : null}
      ...
      <Sheet open={addOpen} onOpenChange={setAddOpen}>
        <SheetContent className="w-full overflow-y-auto border-border bg-white sm:max-w-xl">
          <SheetHeader className="border-b border-border-soft px-5 py-5"><SheetTitle className="text-xl">Add application</SheetTitle><SheetDescription>Save the role now; KithNode will keep the next action visible.</SheetDescription></SheetHeader>
          <form onSubmit={addApplication} className="space-y-4 px-5 pb-8">
            ...
            <button disabled={saving} className="min-h-11 w-full bg-primary px-4 font-bold text-white disabled:opacity-50">{saving ? "Saving…" : "Add to Applications"}</button>
          </form>
        </SheetContent>
      </Sheet>
      ...
      <Sheet open={Boolean(selected)} onOpenChange={(open) => !open && setSelectedId(null)}>
        <SheetContent className="w-full overflow-y-auto border-border bg-white sm:max-w-2xl">
          {selected ? <>
            <SheetHeader className="border-b border-border-soft px-5 py-5 pr-14"><SheetTitle className="text-xl">{selected.role}</SheetTitle><SheetDescription>{selected.company}{selected.location ? ` · ${selected.location}` : ""}</SheetDescription></SheetHeader>
            ...
            <div className="flex flex-wrap gap-2 px-5"><ExternalListing opportunity={selected} /><button type="button" disabled={saving} onClick={generateResume} className="min-h-11 border border-primary/30 bg-primary-soft px-3 font-bold text-primary"><FilePlus2 className="mr-2 inline h-4 w-4" />Create resume variant</button></div>
            <form onSubmit={saveDetails} className="space-y-4 border-y border-border-soft px-5 py-5">
              ...
              <button disabled={saving} className="min-h-11 bg-primary px-4 font-bold text-white"><Save className="mr-2 inline h-4 w-4" />Save details</button>
            </form>
            <section className="px-5"><h3 className="font-heading text-lg font-semibold text-text-primary">Connected people</h3>{selected.contacts?.length ? <div className="mt-3 divide-y divide-white/[0.08] border-y border-border-soft">{selected.contacts.map((link) => <div key={link.id} className="flex items-center justify-between gap-3 py-3"><div><p className="text-base font-semibold text-text-primary">{[link.contact?.firstName, link.contact?.lastName].filter(Boolean).join(" ") || "Saved contact"}</p><p className="text-sm text-text-secondary">{link.contact?.title || link.reason}</p></div><span className="font-mono text-sm text-success">{link.score}</span></div>)}</div> : <p className="mt-2 text-base text-text-secondary">No contacts attached yet. Use Network or Copilot to find a warm path at this firm.</p>}</section>
            <section className="px-5 pb-5"><h3 className="font-heading text-lg font-semibold text-text-primary">Activity</h3><form onSubmit={addEvent} className="mt-3 space-y-2"><Field label="Timeline note" name="title" required placeholder="Interview scheduled" /><TextArea label="Detail" name="detail" rows={2} /><button className="min-h-11 border border-border px-3 font-bold text-text-primary">Add note</button></form><div className="mt-4 border-l border-border pl-4">{selected.events?.length ? selected.events.map((event) => <div key={event.id} className="relative pb-4 before:absolute before:-left-[21px] before:top-1.5 before:h-2 before:w-2 before:bg-primary"><p className="text-base font-semibold text-text-primary">{event.title}</p><p className="text-sm text-text-secondary">{event.detail}</p><p className="mt-1 font-mono text-[11px] text-text-muted">{displayDate(event.createdAt)}</p></div>) : <p className="text-base text-text-secondary">No activity recorded yet.</p>}</div></section>
            <div className="border-t border-border-soft px-5 py-5"><button type="button" onClick={archiveSelected} className="min-h-11 border border-error/30 px-3 font-bold text-error"><Archive className="mr-2 inline h-4 w-4" />Archive application</button></div>
          </> : null}
        </SheetContent>
      </Sheet>
    </div>
...
function ExternalListing({ opportunity }: { opportunity: Opportunity }) {
  ...
  return <a href={href} target="_blank" rel="noopener noreferrer" className="inline-flex min-h-11 items-center border border-border px-3 font-bold text-text-primary hover:bg-surface-soft">Open official listing<ArrowUpRight className="ml-2 h-4 w-4" /></a>;
}

function EmptyApplications({ onAdd }: { onAdd: () => void }) {
  return <div className="mx-auto flex min-h-[430px] max-w-2xl items-center px-5"><div className="w-full border-y border-border py-10 text-left"><BriefcaseBusiness className="h-8 w-8 text-primary" /><h2 className="mt-4 font-heading text-2xl font-semibold text-text-primary">Start your recruiting pipeline</h2><p className="mt-2 max-w-xl text-base leading-6 text-text-secondary">Add a role you are considering or ask Career Copilot to find five student-eligible internships. Every saved opportunity will appear here with its season, deadline, contacts, resume, and next action.</p><div className="mt-6 flex flex-wrap gap-2"><button type="button" onClick={onAdd} className="min-h-11 bg-primary px-4 font-bold text-white"><Plus className="mr-2 inline h-4 w-4" />Add application</button><Link href="/dashboard?skill=find-internships" className="inline-flex min-h-11 items-center border border-border px-4 font-bold text-text-primary"><Sparkles className="mr-2 h-4 w-4 text-primary" />Find internships</Link></div></div></div>;
}
...
function ApplicationTable({ items, onOpen, onStatus }: { items: Opportunity[]; onOpen: (id: string) => void; onStatus: (item: Opportunity, status: OpportunityStatus) => void }) {
  return <div className="mx-auto w-full max-w-[1180px] p-5 sm:px-8"><div className="hidden overflow-x-auto rounded-2xl border border-border bg-white md:block"><table className="w-full min-w-[1050px] border-collapse text-left"><thead className="bg-surface-soft"><tr>{["Company / role", "Status", "Priority", "Deadline", "Fit / network", "Resume", "Next action", "Contacts", "Updated"].map((label) => <th key={label} className="border-b border-border-soft px-3 py-3 font-mono text-[11px] uppercase tracking-[0.1em] text-text-muted">{label}</th>)}</tr></thead><tbody>{items.map((item) => <tr key={item.id} onClick={() => onOpen(item.id)} className="cursor-pointer border-b border-border-soft hover:bg-surface-soft"><td className="px-3 py-3"><div className="mb-1"><StatusBadge tone="info">{opportunityTypeLabel(item.opportunityType || "job")}</StatusBadge></div><p className="text-base font-semibold text-text-primary">{item.company}</p><p className="text-sm text-text-secondary">{item.role}{item.location ? ` · ${item.location}` : ""}</p></td><td className="px-3 py-3" onClick={(event) => event.stopPropagation()}><select aria-label={`Status for ${item.company}`} value={item.status} onChange={(event) => void onStatus(item, event.target.value as OpportunityStatus)} className="min-h-11 border border-border bg-canvas px-2 text-sm text-text-primary">{OPPORTUNITY_STATUSES.map((status) => <option key={status} value={status}>{statusLabel(status)}</option>)}</select></td><td className="px-3 py-3"><StatusBadge>{item.priority}</StatusBadge></td><td className="px-3 py-3 font-mono text-sm text-text-secondary">{displayDate(item.deadline)}</td><td className="px-3 py-3 font-mono text-sm"><span className="text-primary">{item.fitScore || "—"}</span><span className="text-text-muted"> / </span><span className="text-success">{item.networkScore || "—"}</span></td><td className="px-3 py-3 text-sm text-text-secondary">{item.resumeId ? "Attached" : "—"}</td><td className="max-w-[220px] px-3 py-3 text-sm text-text-secondary"><span className="line-clamp-2">{item.nextAction || "Add next action"}</span></td><td className="px-3 py-3 font-mono text-sm text-text-secondary">{item.contacts?.length || 0}</td><td className="px-3 py-3 font-mono text-[12px] text-text-muted">{displayDate(item.lastActivityAt)}</td></tr>)}</tbody></table></div><div className="space-y-2 md:hidden">{items.map((item) => <button key={item.id} type="button" onClick={() => onOpen(item.id)} className="w-full border border-border-soft bg-white p-4 text-left"><div className="flex items-start justify-between gap-3"><div><div className="mb-1"><StatusBadge tone="info">{opportunityTypeLabel(item.opportunityType || "job")}</StatusBadge></div><p className="text-base font-semibold text-text-primary">{item.company}</p><p className="text-sm text-text-secondary">{item.role}</p></div><StatusBadge tone={statusTone(item.status)}>{statusLabel(item.status)}</StatusBadge></div><div className="mt-4 grid grid-cols-2 gap-3 text-sm text-text-secondary"><span><CalendarClock className="mr-1 inline h-4 w-4" />{displayDate(item.deadline)}</span><span><Users className="mr-1 inline h-4 w-4" />{item.contacts?.length || 0} contacts</span></div>{item.nextAction ? <p className="mt-3 border-t border-border-soft pt-3 text-sm text-text-primary">Next: {item.nextAction}</p> : null}</button>)}</div></div>;
}
