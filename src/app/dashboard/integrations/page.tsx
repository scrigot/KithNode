...
interface ServiceState {
  ...
  optional?: boolean;
  descriptor?: {
    status: "configured" | "connected" | "degraded" | "expired" | "unavailable" | "disabled";
...
export default function IntegrationsPage() {
  ...
  useEffect(() => {
    ...
    if (oauthError) setError("That account did not finish connecting. Check the callback URL, then reconnect and test it here.");
  ...
  return (
    <div className="mx-auto max-w-5xl p-5">
      ...
      {error && <div className="mb-4 border border-error/30 bg-error-soft p-3 text-sm text-error">{error}</div>}
      <div id="search"><JobSourceSettings /></div>
      <section id="ai-readiness" className="mb-5 rounded-xl border border-border-soft bg-white p-4">
        <h2 className="font-heading text-xl font-semibold text-text-primary">AI and data readiness</h2>
        <p className="mt-1 text-sm text-text-secondary">Safe capability status only. Secret values never leave the server.</p>
        <div className="mt-3 grid gap-2 sm:grid-cols-2 lg:grid-cols-4">
          {services.map((service) => (
            <div key={service.id} className="rounded-xl border border-border-soft bg-surface-soft p-3">
              <p className="flex items-center gap-1.5 text-sm font-medium">
                <span className={`h-2 w-2 rounded-full ${service.configured ? "bg-success" : service.optional ? "bg-text-faint" : "bg-warning"}`} />
                {service.label}
              </p>
              <p className="mt-1 text-sm leading-6 text-muted-foreground">
                {service.configured || service.optional ? service.validation : service.descriptor?.recoveryAction || "A founder must restore this server capability."}
              </p>
              <p className="mt-2 text-xs capitalize text-text-faint">{service.descriptor?.status || (service.configured ? "configured" : "unavailable")}</p>
            </div>
          ))}
        </div>
        ...
        <div className="mt-4 border-t border-border-soft pt-4">
          ...
          {[
            ...
            return <div key={item.label} className="mt-2 grid items-center gap-2 sm:grid-cols-[150px_minmax(0,1fr)_auto]"><span className="text-[11px] text-muted-foreground">{item.label}</span><code className="min-w-0 overflow-hidden text-ellipsis border border-border bg-surface-soft p-2 text-[11px]">{item.callback}</code><button type="button" onClick={() => navigator.clipboard.writeText(item.callback)} className="inline-flex items-center justify-center gap-1 border border-border px-2 py-2 text-[11px]"><Copy size={11} />Copy</button></div>;
          })}
        </div>
      </section>
      <section id="browser-companion" className="mb-5 rounded-xl border border-border-soft bg-white p-4">
        <div className="flex items-start justify-between gap-3">
          ...
          <button type="button" onClick={createExtensionToken} disabled={Boolean(busy)} className="rounded-lg bg-primary px-3 py-2 text-xs font-semibold text-white disabled:opacity-50">Create token</button>
        </div>
        {newExtensionToken && <div className="mt-4 border border-warning/30 bg-warning-soft p-3"><p className="text-xs text-warning">Copy this now; it cannot be shown again.</p><div className="mt-2 flex gap-2"><code className="min-w-0 flex-1 overflow-hidden text-ellipsis border border-border bg-surface-soft p-2 text-xs">{newExtensionToken}</code><button type="button" onClick={() => navigator.clipboard.writeText(newExtensionToken)} className="flex items-center gap-1 border border-border px-2 text-xs"><Copy size={12} />Copy</button></div></div>}
        <div className="mt-3 space-y-2">{extensionTokens.filter((token) => !token.revokedAt).map((token) => <div key={token.id} className="flex items-center justify-between border border-border-soft bg-surface-soft p-3 text-xs"><div><p className="font-medium">{token.name}</p><p className="mt-0.5 text-muted-foreground">{token.lastUsedAt ? `Last used ${new Date(token.lastUsedAt).toLocaleString()}` : "Never used"}</p></div><button type="button" onClick={() => revokeExtensionToken(token.id)} disabled={Boolean(busy)} className="text-error disabled:opacity-50">Revoke</button></div>)}</div>
      </section>
      <div className="grid gap-4 md:grid-cols-2">
        {items.map((item) => {
          ...
          return (
            <section key={item.provider} className="rounded-xl border border-border-soft bg-white p-4">
              <div className="flex items-start justify-between gap-3">
                <div><h2 className="font-semibold">{info.name}</h2><p className="text-xs text-muted-foreground">{info.detail}</p></div>
                {connected ? <span className="flex items-center gap-1 text-xs text-success"><CheckCircle2 size={13} />{item.connection?.status}</span> : null}
              </div>
              {!item.configured && <p className="mt-4 text-xs text-warning">Provider client ID, secret, or token-encryption key is missing.</p>}
              {connected ? (
                <div className="mt-4 space-y-3">
                  <div className="rounded-xl border border-border-soft bg-surface-soft p-3 text-xs">
                    ...
                    {item.connection?.lastError && <p className="mt-1 text-error">The provider needs attention. Reconnect, then validate this account.</p>}
                  </div>
                  <div className="flex flex-wrap gap-2">
                    ...
                    <button onClick={() => inspect(item.provider)} disabled={Boolean(busy)} className="rounded-xl border border-border px-3 py-2 text-xs disabled:opacity-50">Preview data</button>
                    <button onClick={() => disconnect(item.provider)} disabled={Boolean(busy)} className="flex items-center gap-1 border border-error/30 px-3 py-2 text-xs text-error disabled:opacity-50"><Unplug size={12} /> Disconnect</button>
                  </div>
