...
import { computeLeaderboard } from "@/lib/kith/leaderboard";
...
export async function GET(
  ...
  try {
    ...
    // Snapshot leaderboard — no time window (pipeline progression + contacts added).
    return NextResponse.json({ rows: await computeLeaderboard(id, session.user.id) });
  } catch (err) {
